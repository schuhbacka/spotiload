...
static pt::datetime						g_downloadStartTime = 0;
...
//						LOG( "File " << fn << " already exists, skipping download" );
		...
		case eFinalizeTrack:

			{
				...
				const pt::datetime realDuration = pt::now() - g_downloadStartTime;

				const pt::datetime speedPercent = trackDuration * 100 / realDuration;
		...
		sp_session_process_events(g_session,&nextTimeout);
		pt::psleep(std::min(250,nextTimeout));
	}
