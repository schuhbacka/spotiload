...
{
private:
	class AudioData : public pt::message
	{
	public:
		std::vector<short> m_data;

		AudioData( std::vector<short>& _data ) : pt::message(0)
		{
			m_data.swap(_data);
		}
	};

	class Worker : public pt::thread
	{
		pt::jobqueue& m_queue;
		AudioWriter& m_writer;
	public:
		Worker( pt::jobqueue& _queue, AudioWriter& _writer ) : pt::thread(false), m_queue(_queue), m_writer(_writer)
		{
		}
		...
		void execute()
		{
			while( true )
			{
				pt::message* msg = m_queue.getmessage();
				if( msg->id == 0 )
				{
